...
int endwith(const char str[], const char substr[]) {
	...
	return memcmp((char*) (str + lenStr - lenSubstr), substr, lenSubstr) == 0;
}

static char *OCTET_SUFFIX[5] = { ".gz", ".bz2", ".deb", ".gpg", NULL };
/**********************************************************************/
/* gz
 * bz2
 *  deb
...
/**********************************************************************/
const char *getMime(const char *filename) {
	static const char* octet = "binary/octet-stream";
	static const char* plain = "text/plain";

	char** suffix = OCTET_SUFFIX;
	while (*suffix != NULL) {
		printf("check end with:%s\n", *suffix);
		if (endwith(filename, *suffix))
			return octet;
		suffix++;
	}
//	if(endwith(filename, "gz")
//			||endwith(filename, "bz2")
//			||endwith(filename, "deb")
//			||endwith(filename, "gpg"))
//		return octet;
	return plain;
}
...
int startup(u_short *port) {
	...
	if (*port == 0) /* if dynamically allocating a port */
	{
		...
		if (getsockname(httpd, (struct sockaddr *) &name, (void *) &namelen)
				== -1)
